      ...
      {!isPro ? (
            ...
            {[1,2,3].map((_, i) => (
              <div key={i} className={`p-2 rounded-xl border flex items-center gap-2 ${T.historyBg}`}>
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-[9px] sm:text-xs font-medium truncate ${T.text}`}>{"XXXXX XXXXX"}</p>
                  <p className={`text-[8px] sm:text-[9px] truncate ${T.label}`}>{"xxx@xxx.com"}</p>
                </div>
                <span className={`text-[8px] font-medium px-1.5 py-0.5 rounded uppercase tracking-widest bg-blue-100 text-blue-700`}>{"member"}</span>
              </div>
            ))}
          </div>
          <div className="absolute inset-0 flex flex-col items-center justify-center z-10">
            <Lock size={20} className={`${T.label} mb-3`} />
            <p className={`text-xs font-medium ${T.text} mb-1`}>Members panel is Pro only</p>
            <button onClick={onUpgrade} className="mt-3 px-4 py-2 bg-amber-500 hover:bg-amber-400 text-slate-900 rounded-xl font-medium text-[9px] uppercase tracking-widest cursor-pointer transition">Upgrade</button>
          </div>
        </div>
      ) : (
        <div className="space-y-2 max-h-[calc(70vh-3.5rem)] sm:max-h-[calc(80vh-4rem)] overflow-y-auto pr-1">
          ...
          {members.map((m, i) => {
            ...
            const nameText = (m?.name || m?.displayName || "").trim() || (m?.email && m.email.includes("@") ? m.email.split("@")[0] : "Unknown");
            return (
              <div key={m.email || i} className={`p-2 rounded-xl border flex items-center gap-2 ${T.historyBg}`}>
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-[9px] sm:text-xs font-medium truncate ${T.text}`}>{nameText}</p>
                  <p className={`text-[8px] sm:text-[9px] truncate ${theme === "light" ? "text-slate-500" : "text-slate-300"}`}>{m.email}</p>
                </div>
                <span className={`text-[8px] font-medium px-1.5 py-0.5 rounded uppercase tracking-widest ${m.role === "admin" ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"}`}>{m.role}</span>
              </div>
            );
          })}
        </div>
