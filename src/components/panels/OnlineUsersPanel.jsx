    ...
    >
      ...
      {!isPro ? (
        <>
          <div className="space-y-2">
            ...
            {[1, 2].map((i) => (
              <div key={i} className={`p-2 rounded-xl border flex items-center gap-2 ${T.historyBg}`}>
                ...
                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-medium blur-[5px] opacity-40
                  ${theme === "light" ? "bg-blue-100 text-blue-700" : "bg-blue-500/20 text-blue-200"}`}
                >?</div>
                <div className="flex-1 min-w-0">
          ...
          <button onClick={onUpgrade} className="w-full mt-4 py-2 rounded-xl font-medium text-[9px] uppercase tracking-widest cursor-pointer bg-amber-500/10 text-amber-400 hover:bg-amber-500/20 transition border border-amber-500/20">
